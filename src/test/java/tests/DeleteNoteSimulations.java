...
public class DeleteNoteSimulations extends Simulation {
    ...
    private final ScenarioBuilder scenario = scenario("Create, Delete Note and Delete User Scenario")
            .exec(session -> {
            ...
            .exec(http("Delete Note Request")
                    ...
                    .check(
                            jsonPath("$.success").is("true"),
                            jsonPath("$.status").is("200"),
                            jsonPath("$.message").is("Note successfully deleted")
                    )
            )
