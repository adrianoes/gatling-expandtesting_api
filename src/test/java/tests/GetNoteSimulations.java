...
public class GetNoteSimulations extends Simulation {
    ...
    private final ScenarioBuilder scenario = scenario("Get Note by ID Scenario")
            .exec(session -> {
            ...
            .exec(http("Get Note by ID Request")
                    .get("/notes/api/notes/#{noteId}")
                    .header("accept", "application/json")
                    .header("x-auth-token", "#{authToken}")
                    .check(
                            jsonPath("$.success").is("true"),
                            jsonPath("$.status").is("200"),
                            jsonPath("$.message").is("Note successfully retrieved"),
                            jsonPath("$.data.id").is(session -> session.getString("noteId")),
                            jsonPath("$.data.created_at").is(session -> session.getString("noteCreatedAt")),
                            jsonPath("$.data.updated_at").is(session -> session.getString("noteUpdatedAt")),
                            ...
                            jsonPath("$.data.completed").is("false")
                    )
            )
